  ...
  <header className="fixed top-0 left-0 right-0 h-20 bg-black/80 backdrop-blur-md z-50 flex items-center justify-between px-6 border-b border-white/10">
    <div className="flex items-center gap-4">
      <a href={`https://${translations.headerLink[lang]}`} target="_blank" rel="noopener noreferrer">
        <img
          src="https://drive.google.com/thumbnail?id=153FZr2r59JJLnApFRrO73DIzcGT72hXv&sz=w200"
          alt="Logo"
        ...
        {(Object.keys(Language) as Language[]).map((l) => (
          <button
            ...
            className={`px-4 py-2 rounded-lg text-sm font-black transition ${
              lang === l ? 'bg-red-600 text-white shadow-lg shadow-red-600/30' : 'bg-white/10 text-white hover:bg-white/20'
            }`}
          >
            {l}
          </button>
...
const Footer: React.FC<{ lang: Language }> = ({ lang }) => (
  <footer className="fixed bottom-0 left-0 right-0 h-24 bg-black/95 backdrop-blur-lg z-50 flex items-center justify-between px-10 border-t border-white/10 text-sm text-white">
    <a
      href="https://www.youtube.com/@walk-around-the-world"
      target="_blank"
      rel="noopener noreferrer"
      className="hover:text-red-500 transition flex items-center gap-4 font-black uppercase tracking-widest group"
    >
    ...
    <div className="text-center">
      <a
        href="https://www.kinderschutz24.ch"
        target="_blank"
        rel="noopener noreferrer"
        className="hover:text-red-500 transition underline font-black uppercase tracking-tighter text-lg block"
      >
    ...
    <a
      href="https://kinderschutz24.payrexx.com/de/pay?cid=48744b73"
      target="_blank"
      rel="noopener noreferrer"
      className="hover:text-red-500 transition font-black uppercase tracking-[0.2em] text-lg border-b-4 border-red-600 pb-1"
    >
...
const AudioInput: React.FC<{ onResult: (text: string) => void }> = ({ onResult }) => {
  ...
  return (
    <button
      ...
      className={`p-5 rounded-2xl transition shadow-xl ${
        isRecording ? 'bg-red-600 animate-pulse text-white' : 'bg-white/10 text-white hover:bg-white/20'
      }`}
      title="Voice Input"
    >
...
export default function App() {
  ...
  const handleKeySelection = () => {
    ...
    if (!key) {
      alert('Bitte füge deinen Gemini API Key ein.');
      return;
    }
    localStorage.setItem('ytai_gemini_api_key', key);
    setHasKey(true);
  };
  ...
  const handleFileUpload = (
    ...
    target: 'detailImages' | 'environmentImages' | 'protagonistImages' | number
  ) => {
      ...
      r.onload = (ev) => {
        if (typeof ev.target?.result === 'string') {
          if (typeof target === 'number') {
            ...
            setState((s) => ({
              ...s,
              [target]: [...(s[target] as string[]), ev.target!.result as string].slice(0, 10),
            }));
          }
        }
      };
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center text-white text-center p-6">
        ...
        <p className="max-w-md text-white/70 mb-6">
          ...
          Testphase werden 2 CHF / Monat verrechnet. Du wirst darauf nach Deine Google API-Key gefragt, damit die KI das Bild erstellen kann. Die Key findest Du hier: https://console.cloud.google.com/. Und falls Du eine Anleitung benötigst, wähle diese Domain: https://drive.google.com/file/d/1HNgkNXDb0R5ewSqdP6YHeIk90wOzq8Sm/view?usp=sharing
        </p>
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-6 text-center">
        <img
          ...
          className="w-64 mb-12 opacity-80"
          alt="Logo"
        />
        ...
        <input
          ...
          className="w-full max-w-xl bg-white/10 border-2 border-white/30 rounded-3xl px-8 py-5 text-white text-xl focus:ring-4 focus:ring-red-600/50 outline-none transition mb-6"
        />

        <button
          onClick={handleKeySelection}
          className="bg-red-600 text-white px-12 py-6 rounded-full font-black text-xl hover:bg-red-700 transition shadow-2xl uppercase tracking-widest"
        >
          Aktivieren & Starten
        </button>
      </div>
  ...
  return (
    <div className="min-h-screen pt-24 pb-32 px-6 text-white max-w-6xl mx-auto relative z-10">
      ...
      {selectedImage && (
        <div
          className="fixed inset-0 z-[100] bg-black/95 flex items-center justify-center p-8 cursor-zoom-out animate-in fade-in"
          onClick={() => setSelectedImage(null)}
        >
          <button className="absolute top-8 right-8 text-white hover:text-red-500 transition z-[110]">
      ...
      {step === 1 ? (
        <div className="space-y-16 animate-in fade-in slide-in-from-bottom-8 duration-700 pb-20">
          ...
          <section className="bg-white/5 backdrop-blur-xl p-12 rounded-[2.5rem] border-2 border-white/20 relative shadow-2xl">
            ...
            <div className="text-center mb-16">
              ...
              <input
                className="w-full max-w-3xl bg-white/10 border-2 border-white/30 rounded-3xl px-8 py-5 text-white text-xl focus:ring-4 focus:ring-red-600/50 outline-none transition"
                value={state.youtubeLink}
                onChange={(e) => setState({ ...state, youtubeLink: e.target.value })}
            ...
            <div className="grid md:grid-cols-2 gap-12">
              <div className="space-y-6">
                ...
                <div className="flex gap-5">
                  <input
                    className="flex-1 bg-white/10 border-2 border-white/20 rounded-2xl px-8 py-6 text-white text-xl outline-none"
                    value={state.videoTopic}
                    onChange={(e) => setState({ ...state, videoTopic: e.target.value })}
                  />
                  <AudioInput onResult={(t) => setState((s) => ({ ...s, videoTopic: s.videoTopic + t }))} />
                </div>
              </div>
              ...
              <div className="space-y-6">
                ...
                <div className="flex gap-5">
                  <textarea
                    rows={4}
                    className="flex-1 bg-white/10 border-2 border-white/20 rounded-2xl px-8 py-6 text-white text-xl outline-none resize-none"
                    value={state.storyDetails}
                    onChange={(e) => setState({ ...state, storyDetails: e.target.value })}
                  />
                  <AudioInput onResult={(t) => setState((s) => ({ ...s, storyDetails: s.storyDetails + t }))} />
                </div>
              </div>
            </div>
            ...
            <div className="mt-12 space-y-6">
              ...
              <div className="flex gap-5">
                <input
                  className="flex-1 bg-white/10 border-2 border-white/20 rounded-2xl px-8 py-6 text-white text-xl outline-none"
                  value={state.importantDetails}
                  onChange={(e) => setState({ ...state, importantDetails: e.target.value })}
                />
              ...
              <div className="grid grid-cols-4 md:grid-cols-6 gap-5 mt-6">
                <div className="aspect-square border-3 border-dashed border-white/40 rounded-3xl flex items-center justify-center relative cursor-pointer hover:border-red-600 transition bg-white/5">
                  <input type="file" multiple className="absolute inset-0 opacity-0" onChange={(e) => handleFileUpload(e, 'detailImages')} />
                  <span className="text-3xl">+</span>
                </div>
                {state.detailImages.map((img, idx) => (
                  <div key={idx} className="aspect-square rounded-3xl overflow-hidden relative border-2 border-white/20 group">
                    <img src={img} className="w-full h-full object-cover" alt="" />
                    <button
                      onClick={() => removeFile(idx, 'detailImages')}
          ...
          <div className="flex justify-center">
            <button
              ...
              className="group relative bg-white text-black font-black px-24 py-10 rounded-full text-4xl hover:scale-110 active:scale-95 transition shadow-2xl uppercase tracking-tighter"
            >
              <span>{isGenerating ? translations.pleaseWait[lang] : translations.createButton[lang]}</span>
            </button>
          </div>
        </div>
      ) : (
        <div className="space-y-20 animate-in fade-in zoom-in-95 duration-700 pb-32">
          <button
            onClick={() => setStep(1)}
            className="flex items-center gap-4 font-black text-red-500 uppercase tracking-[0.3em] text-xl transition hover:translate-x-[-8px]"
          >
            <svg className="w-10 h-10" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="grid md:grid-cols-2 gap-12">
            {results.map((res) => (
              <div
                key={res.id}
                className="bg-white/5 backdrop-blur-2xl rounded-[3rem] border-2 border-white/20 overflow-hidden flex flex-col shadow-2xl group transition hover:border-red-600/50"
              >
                <div
                ...
                >
                  ...
                  {!res.isGenerating && res.url && (
                    <a
                      href={res.url}
                      download
                      className="absolute bottom-8 right-8 bg-white text-black p-6 rounded-full z-10 hover:scale-125 transition"
                    ...
                    <div className="flex gap-5 relative">
                      ...
                      <div className="w-16 h-16 bg-white/10 border-2 border-white/20 rounded-2xl flex items-center justify-center relative group/upload overflow-hidden">
                        {refinementImages[res.id] ? (
                          <>
                            <img src={refinementImages[res.id]} className="w-full h-full object-cover" alt="" />
                            <button
                              onClick={(e) => {
                                e.stopPropagation();
                                removeFile(0, res.id);
                              }}
                              className="absolute inset-0 bg-red-600/80 flex items-center justify-center opacity-0 group-hover/upload:opacity-100 transition"
                            >
                          ...
                          <>
                            ...
                            <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                              <path
                                ...
                                strokeLinejoin="round"
                                strokeWidth="2.5"
                                d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h14a2 2 0 002-2V6a2 2 0 00-2-2H4a2 2 0 00-2 2v12a2 2 0 002 2z"
                              ></path>
                            </svg>
                          </>
                        )}
                      ...
                      <button
                        onClick={() => handleRefine(res.id)}
                        className="w-16 h-16 bg-red-600 text-white rounded-2xl flex items-center justify-center shadow-xl hover:bg-red-700 transition active:scale-90"
                      >
                        <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
